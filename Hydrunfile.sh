...
apt update && DEBIAN_FRONTEND=noninteractive apt install -y --no-install-recommends build-essential subversion python3-dev python3-pip libncurses5-dev libxml2-dev libedit-dev swig graphviz xz-utils ninja-build gcc g++ sed curl wget
...
tar -zvcf ./xeus-clangd.$version.$(uname -m).tar.gz *
rm -rf /tmp/build
